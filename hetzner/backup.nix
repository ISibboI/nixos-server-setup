{ config, pkgs, ... }: {
  ...
  systemd.services."backup-daily" = {
    script = ''
      ...
      readonly SOURCE_CONFIGS=("/home/syncthing:syncthing" "/syncthing_shared:syncthing_shared" "/var/lib/immich:immich" "/var/lib/nextcloud:nextcloud" "/var/lib/hass/backups:hass" "/var/lib/jellyfin:jellyfin" "/var/lib/private/actual:actual" "/var/lib/navidrome:navidrome" "/var/lib/reitti:reitti" "/usr/lib/endurain:endurain" "/var/lib/audiobookshelf:audiobookshelf")
  ...
  systemd.services."backup-duperemove" = {
    script = ''
      ...
      ${pkgs.coreutils}/bin/nice -n 10 ${pkgs.duperemove}/bin/duperemove -rhd --hashfile /backup/duperemove_hashfile /backup/monthly /backup/yearly
    '';
    serviceConfig = {
      Type = "oneshot";
      User = "root";
